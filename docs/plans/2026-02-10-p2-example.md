...
> Status update (February 10, 2026): framework w repo to `PFrame` (dawniej `P1`), dlatego nazwy klas/plików w tym planie należy czytać jako historyczne.
> Aktualne API zgodne z `src/PFrame.php`.
    ...
    <nav>
        ...
        <span class="right">
            <?php if ($is_logged_in ?? false): ?>
                ...
                | <form method="post" action="/logout" style="display:inline"><?= $csrf_input ?><button type="submit" style="background:none;border:none;color:#0066cc;cursor:pointer;padding:0;font:inherit">Wyloguj</button></form>
            <?php else: ?>
...
NOTE: Nie uruchamiaj serwera jeszcze — index.php require'uje Auth/AdminController które będą w Task 5/6.
...
7. POST /logout (z CSRF) → redirect do / z flash "Wylogowano"
